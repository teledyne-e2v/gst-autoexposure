...
int algorithm_analog_gain(int target, int global_mean, int analog_gain)
{
	...
	else if(new_analog_gain > get_control_max("digital_gain"))
	{
		new_analog_gain = get_control_max("digital_gain");
		set_control("digital_gain", new_analog_gain);
...
void algorithm_without_exposition_v3(float global_mean, int latency, int target)
{
	...
	if (proc_once2 == 0)
	{
		...
		{
			int expected = algorithm_digital_gain(target,global_mean,analog_gain);
			if(expected != 0)
			{
				algorithm_analog_gain(target,global_mean,digital_gain);
			}
		}
		proc_once2 = 1;
	}
...
void algorithm_with_exposition_v3(float global_mean, int maxExp, int latency, int lowerbound, int upperbound)
{
	...
	if (global_mean < lowerbound && (proc_Once == 0 || exp1 != maxExp))
	{
		...
		else
		{
			...
			algorithm_without_exposition_v3(delta * global_mean, latency, lowerbound, upperbound);
		}
	}
	else if (global_mean < lowerbound && proc_Once > latency)
	{
		algorithm_without_exposition_v3(global_mean, latency, lowerbound, upperbound);
	}
