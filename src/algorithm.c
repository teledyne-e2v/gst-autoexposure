...
float algorithm_without_exposition(float global_mean, int latency, int target, int max_analog_gain, bool toggle_digital_gain, int min_digital_gain, int max_digital_gain, int tolerance)
{
	...
	if (proc_once2 == 0)
	{
		if (target > global_mean) // if the targeted mean is greater than the global mean, we start to change the analogic gain (because he should produce less noise than digital gain)
		{
			...
			{
				expected = algorithm_digital_gain(target, expected, digital_gain,min_digital_gain, max_digital_gain);
			}
		}
		else // if the targeted mean is lower than the global mean, we start to change the digital gain (to minimize the noise)
		{
			...
			{
				expected = algorithm_digital_gain(target, global_mean, digital_gain,min_digital_gain, max_digital_gain);
				if ((expected > target + tolerance || expected < target - tolerance) || !toggle_digital_gain)
				{
...
void algorithm_with_exposition(float global_mean, int latency, int target, int max_exposition, int max_analog_gain, bool toggle_digital_gain, int min_digital_gain, int max_digital_gain, int tolerance)
{
	...
	if (proc_Once == 0)
	{
		if (target > global_mean) // if the targeted mean is greater than the global mean, we start to change the analogic gain (because he should produce less noise than digital gain)
		{
			...
			{
				proc_once2 = 0;
				expected = algorithm_without_exposition(expected, latency, target, max_analog_gain, toggle_digital_gain,min_digital_gain,  max_digital_gain,tolerance);
